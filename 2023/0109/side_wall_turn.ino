...
if((R_distance+L_distance+robot_width >= mazor_width-mazor_width_tolerance) && (R_distance+L_distance+robot_width <= mazor_width+mazor_width_tolerance)){
  ...
  if(L_distance_error < -200){
    L_distance_error = -100;
  }
  else if(L_distance_error > 200){
    L_distance_error = 100;
  }
  
  
    if(F_distance < 50 && index == -5555){
    M_CTL(0,0,10);
    M_CTL(speed_nomal,-speed_nomal,950);
      
    
    M_CTL(0,0,10);
    M_CTL(speed_nomal,-speed_nomal,480);
    int turn_dix =0;
    ...
    while(1){
      ...
      else if(F_distance <50 && index ==-5555){
        if(turn_dix % 2 == 0){
          M_CTL(0,0,10);
          M_CTL(-speed_nomal,speed_nomal,480);
          turn_dix++;
  ...
  else if(distance_error>mazor_width_tolerance && L_distance_error < -10){
        //좌측으로 붙기.
...
switch(index){
    ...
    //우회전
    case 1: 
      M_CTL(speed_nomal,0.45*speed_nomal,10);
      break;
    case 2:
      M_CTL(speed_nomal,0.35*speed_nomal,10);
      break;
    case 3:
      M_CTL(speed_nomal,0.3*speed_nomal,10);
      break;
    case 4: 
      while(1){
      M_CTL(speed_nomal,0.2*speed_nomal,10);
      index = read_line_sensor();
      if((index != 4)&&(index != 2)){
        break;
        }
    ...
    if(F_distance <=60){
      ...
      M_CTL(speed_nomal,speed_nomal,10);
      break;
    //좌회전
    case -1: 
      M_CTL(0.45*speed_nomal,speed_nomal,10);
      break;
    case -2: 
      M_CTL(0.35*speed_nomal,speed_nomal,10);
      break;
    case -3: 
      M_CTL(0.3*speed_nomal,speed_nomal,10);
      break;
    case -4: 
      while(1){
      M_CTL(0.2*speed_nomal,speed_nomal,10);
      index = read_line_sensor();
      if((index != -4)&&(index != -2)){
        break;
        }
