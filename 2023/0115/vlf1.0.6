...
float T_turn90=(850.0*100.0/(float)spd_N*0.8);
int obstacle_num =0;
int wall_num =0;
...
void cos_line(){
  switch(index){
    ...
    //우회전
    case 1: 
      M_CTL(spd_N*1.2,0.5*spd_N,1);
      break;
    case 2:
      M_CTL(spd_N*1.5,0.3*spd_N,1);
      break;
    case 3:
      M_CTL(spd_N*1.5,0.25*spd_N,1);
      break;
    case 4: 
   //   while(1){
      M_CTL(spd_N*1.65,0.15*spd_N,1);
    /*  index = read_line_sensor();
      read_sonar_sensor();
      if((index != 4)&&(index != 3))
       break;
    ...
    //좌회전
    case -1: 
      M_CTL(0.5*spd_N,spd_N*1.2,1);
      break;
    case -2: 
      M_CTL(0.3*spd_N,spd_N*1.5,1);
      break;
    case -3: 
      M_CTL(0.25*spd_N,spd_N*1.5,1);
      break;
    case -4: 
      //while(1){
      M_CTL(0.15*spd_N,spd_N*1.65,1);
     /* index = read_line_sensor();
     read_sonar_sensor();
     if((index != -4)&&(index != -3) ){
    break;
...
void line_set_mid(){
  ...
  index = read_line_sensor();
    ...
    else if(L_distance_error > 10 || R_distance <(mazor_width/2)-mazor_width_tolerance){
      //좌측으로 붙기.
      M_CTL(spd_N*0.5,1.3*spd_N,100);
    }
    else if(L_distance_error < -10 || L_distance < (mazor_width/2)-mazor_width_tolerance){
      //우측으로 붙기.
      M_CTL(spd_N*1.3,0.5*spd_N,100);
    }   
}
void cos_miro(){
 ...
 read_sonar_sensor();
  ...
  switch(turn_num){
	case -1:
        ...
        //좌측으로 붙기.
        M_CTL(spd_N*0.7,1.3*spd_N,200);
      }
      else if((L_distance_error < -10) || (L_distance < (mazor_width/2)-mazor_width_tolerance)){
        //우측으로 붙기.
        M_CTL(spd_N*1.3,0.7*spd_N,200);
      }
		  break;
    case 0:
      turn_R();
